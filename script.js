document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (pattern) {
        ...
        if (!regex.test(value)) {
          showFieldError(field, field.title || 'Please match the required format.');
          isValid = false;
          if (!firstInvalidField) firstInvalidField = field;
          return;
        }
  ...
  function getFieldLabel(field) {
    ...
    if (wrappingLabel) {
      ...
      clone.querySelectorAll('input, textarea, select').forEach(function (el) { el.remove(); });
  ...
  function initScrollAnimations() {
    const elements = document.querySelectorAll(
      '.fade-in, [data-animate], .animate-on-scroll, .section-title, .feature-card, .pricing-card, .testimonial-card, .hero-content, .about-content, .service-item, .team-card, .blog-card, .portfolio-item, .stat-item, .cta-block'
    );
  ...
  // Re-run for dynamically added elements (MutationObserver)
      ...
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          ...
          const animatables = node.querySelectorAll
            ? node.querySelectorAll('.fade-in, [data-animate], .animate-on-scroll')
            : [];
          if (animatables.length) initScrollAnimations();
        });
      });
    });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "צלילים בקנה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '4c320725-74c3-42c2-b774-1c598e61f994',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '4c320725-74c3-42c2-b774-1c598e61f994';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '4c320725-74c3-42c2-b774-1c598e61f994';
